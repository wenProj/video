            ...
            if(member_id != ""){
              await new Promise(async (carryon2)=> {     
                try {
                  ...
                  //经纬度统计
                  if(des.indexOf("度") <= 0){
                    area_country = await getLonLat(lon,lat);
                    if(area_country == ""){
                      logger.log("info",`------------经纬度抓取为空： ${area_country}------------`);
                    }
                  }
...
const getIp = async(ip)=> {
  ...
  await new Promise(async (carryon7)=> {
    originRequest({url: url,encoding: null,headers: {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.65 Safari/537.36','Accept-Language': 'zh-CN,zh;'}}, (err, res, body) => {
      if(body != undefined){
  ...
  if(times >= 3){
    logger.log("info",`------------IP递归了3次，执行下一个:${ip}------------`);
    console.log(`------------IP递归了3次，执行下一个:${ip}------------`);
    times = 0;
    return area;
  }
  if(area == ""){
...
const getLonLat = async(lon,lat)=> {
  ...
  await new Promise(async (carryon8)=> {
    originRequest({url: latlonUrl,encoding: null,headers: {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.65 Safari/537.36','Accept-Language': 'zh-CN,zh;'}}, (err, res, body) => {
      if(body != undefined){
  ...
  if(timesLonLat >= 3){
    logger.log("info",`-----------经纬度递归了3次，执行下一条：${lon},${lat}------------`);
    console.log(`------------经纬度递归了3次，执行下一条：${lon},${lat}------------`);
    timesLonLat = 0;
    //调用google查询
    // let url = 'http://maps.googleapis.com/maps/api/geocode/json?latlng='+lat+','+lon+'&sensor=false';//纬度,经度
    // await new Promise(async (carryon9)=> {
    //   originRequest({url: url,encoding: null,headers: {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.65 Safari/537.36','Accept-Language': 'zh-CN,zh;'}}, (err, res, body) => {
    //     if(body != undefined){
    //       let json = JSON.parse(iconv.decode(body, 'UTF-8'));
    //       if(json.status != undefined && json.status == 'OK'){
    //         let results = json.results;
    //         area_country = results[0].formatted_address.split(" ")[0]
    //       }
    //     }
    //     carryon9();
    //   });
  ...
  if(area_country == ""){
    timesLonLat++;
    return await getLonLat(lon,lat);
  }else{
    timesLonLat = 0;
    return area_country;
  }
